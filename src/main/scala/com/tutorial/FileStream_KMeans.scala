...
//object FileStream_KMeans {
 ...
 //   Logger.getLogger("org").setLevel(Level.OFF)
  //  Logger.getLogger("akka").setLevel(Level.OFF)
    ...
    val trainingLines = ssc.textFileStream("hdfs://localhost:9000/datadir")


    val trainingLines = ssc.textFileStream("hdfs://localhost:9000/datadir")//.map(Vectors.parse)
    //val testingLines = ssc.textFileStream("src/Resources/kmeans-test.txt")//.map(LabeledPoint.parse)
    ...
    //val trainingLines = ssc.socketTextStream("localhost", 9999, StorageLevel.MEMORY_AND_DISK_SER)
   ...
   // val testingLines = ssc.socketTextStream("localhost", 7777, StorageLevel.MEMORY_AND_DISK_SER)
